/**
 * Shared utility for handling tool calls in streaming responses
...
export function processStreamContent(
  ...
  if (state.isBufferingToolCall) {
    ...
    if (toolCalls.length > 0) {
      ...
      // If we see the closing tag, we can stop buffering
      ...
      // Just clear the buffer if we are not buffering another tool call.
      if (!state.isBufferingToolCall) {
        state.contentBuffer = ''
      }
  ...
  if (state.contentBuffer) {
    // CRITICAL FIX: If we have already emitted a tool call in this stream,
