          ...
          for (const event of events) {
            ...
            if (deltaContent) {
              ...
              if (!isBufferingToolCall) {
                const lastBracketIdx = contentBuffer.lastIndexOf('[')
                if (lastBracketIdx !== -1) {
                  const potentialMarker = contentBuffer.substring(lastBracketIdx)
                  if (marker.startsWith(potentialMarker)) {
                    isBufferingToolCall = true
                    // If we have text before the bracket, send it first
                    if (lastBracketIdx > 0) {
                      const textBefore = contentBuffer.substring(0, lastBracketIdx)
                      const textData = {
                        ...transformedData,
                        choices: [{
                          index: 0,
                    ...
                    continue // Hold the potential marker
                  }
                }
              }
              ...
              if (isBufferingToolCall) {
                ...
                const hasFullMarker = contentBuffer.includes(marker)
                const isPrefix = marker.startsWith(contentBuffer)
            ...
            // CRITICAL FIX: Only push if we are NOT currently buffering a potential tool call
            if (!isBufferingToolCall) {
              isFirstChunk = false
      ...
      flush(callback) {
        if (contentBuffer) {
          ...
          if (toolCalls.length > 0) {
            ...
            // AGGRESSIVE FILTERING: If we found tool calls, DO NOT send the remaining text content.
            // This prevents the model's "self-talk" from confusing the client.
            // Only send content if it's NOT empty and we want to allow mixed content (which we don't for now).
            /* 
            if (cleanContent) {
              const textData = {
                id: responseId,
